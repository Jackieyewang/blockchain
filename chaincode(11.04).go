...
type Member struct{
    ...
    MemberPassword string `json:MemberPassword`               //密码
...
type ResInfo struct {                                         //返回结果信息
    Status bool `json:Status`                                 //密码true or false
...
func (a *CollegeChainCode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
    ...
    } else if fn == "checkPassword"{
        return a.checkPassword(stub,args)
    }
...
func(a *CollegeChainCode) addProInfo(stub shim.ChaincodeStubInterface,args []string) pb.Response{
    ...
    MemberID := args[0]                                         //获得该member当前的状态
    Memberbyte, err := stub.GetState(MemberID)
    if err != nil {
    ...
    err = json.Unmarshal(Memberbyte, &Members)
    if err != nil {
    ...
    ProInfoitem.ProSpeech = args[3]
    ProInfoitem.ProOwnerID = args[4]
    X, err = strconv.Atoi(args[5])                             //string转换为int
    if err != nil {
        return shim.Error("Invalid transaction amount")
    }
    ProInfoitem.ProPrice = X
...
func (t *ResInfo) result() pb.Response {
...
func (a *CollegeChainCode) checkPassword(stub shim.ChaincodeStubInterface, args []string) pb.Response {
    res := &ResInfo{false, ""}
    if len(args) != 2 {
        return shim.Error("Incorrect number of arguments. Expecting function followed by a name and a password")
    }
    MemberID = args[0]
    Password = args[1]
    //获取用户真实密码
    MemberInfobytes, err := stub.GetState(MemberID)
    if err != nil {
        return shim.Error("Failed to get state")
    }
    if Buyerbytes == nil {
        return shim.Error("Member is not exists")                         //检查用户名是否存在
    }
    var MemberInfos MemberInfo
    err = json.Unmarshal(MemberInfobytes, &MemberInfos)
    if err != nil {
        return shim.Error("Failed to decode JSON")
    }
    Password_True = MemberInfos.MemberPassword
    //比较输入密码与真实密码
    if strings.Compare(Password, Password_True) == 0 {
        res.Ok("")
    }
    else{
        res.No("The password is wrong")
    }
    return res.result()
}
